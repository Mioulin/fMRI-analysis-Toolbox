function fmri_glm_estimate()
  ...
  for subID = 1:params.num.subjects
    ...
    for runID = 1:params.num.runs
        % load EPIs
        funcDir = [params.dir.imDir subjectDirName '/' params.dir.epiSubDir num2str(runID) '/'];
        fileNames = spm_select('List', funcDir,params.regex.functional);
        matlabbatch{1}.spm.stats.fmri_spec.sess(runID).scans = cellstr([repmat(funcDir,size(fileNames,1),1) fileNames]);

        % load condition file....
        matlabbatch{1}.spm.stats.fmri_spec.sess(runID).cond = struct('name', {}, 'onset', {}, 'duration', {}, 'tmod', {}, 'pmod', {}, 'orth', {});
        matlabbatch{1}.spm.stats.fmri_spec.sess(runID).multi = cellstr([params.dir.conditionDir 'conditions_' params.glmName '_sub' num2str(subID) '_run' num2str(runID) '.mat']);
        % ...  and motion regressors
        fileName = spm_select('List', funcDir,params.regex.motionregs);
        matlabbatch{1}.spm.stats.fmri_spec.sess(runID).regress = struct('name', {}, 'val', {});
        matlabbatch{1}.spm.stats.fmri_spec.sess(runID).multi_reg = cellstr([funcDir fileName]);

        % set high pass filter
        matlabbatch{1}.spm.stats.fmri_spec.sess(runID).hpf = params.misc.highpass;
    end
