function rdmSet = fmri_rsa_compute_rdmSet_cval(betas,metric)
  %% fmri_rsa_compute_rdmSet(betas,metric)
  %
  % computes rdms at single subject level,
  % ignores within run dissimilarities (NaN)
  % warning: RDMs can be very huge (6 runs, 50conditions per run: 300x300 entries)
  %
  % Timo Flesch, 2019
  ...
  case {'mahalanobis'}
    for ii=1:size(b,1)
      for jj=1:size(b,1)
        dVTrain = fmri_helper_diffVect(b(ii,:),b(jj,:));
...
function dv = fmri_helper_diffVect(bV1,bV2)

  dv = [bV1-bV2];

end

function mcv = fmri_helper_meanCV(bM1,bM2)
  mcv = fmri_helper_diffVect(bM1(1,:),bM2(1,:));
  for ii = 2:size(bM1,1)
    mcv = mcv + fmri_helper_diffVect(bM1(ii,:),bM2(ii,:));
  end
  mcv = mcv./size(bM1,1);
end
